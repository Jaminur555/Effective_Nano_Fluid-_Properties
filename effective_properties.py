...
def eff_prop_mono(rho_w, Cp_w, k_w, mu_w,
    ...
    """
    rho_nf = (1 - phi) * rho_w + phi * rho_n                       # mixture rule

    Cp_nf = ((1 - phi) * rho_w * Cp_w + phi * rho_n * Cp_n) / rho_nf  # energy-weighted

    k_nf = k_w * (                                                 # Maxwell model (spherical particles)
        (k_n + 2*k_w - 2*phi*(k_w - k_n)) /
        (k_n + 2*k_w + phi*(k_w - k_n))
    )

    mu_nf = mu_w / (1 - phi) ** 2.5                                # Brinkman model
...
def eff_prop_di(rho_w, Cp_w, k_w, mu_w,
    ...
    Cp_hf = rhoCp_hf / rho_hf                                      # energy-weighted

    mu_hf = mu_w / (((1 - phi1) ** 2.5) * ((1 - phi2) ** 2.5))     # Brinkman model (both particles)

    # Two-step Maxwell: suspend particle 1 in water, then particle 2 in that mix
    knf = k_w * (
